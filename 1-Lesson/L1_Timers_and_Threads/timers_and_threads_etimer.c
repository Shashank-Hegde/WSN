...
PROCESS_THREAD(timers_and_threads_process, ev, data)
{
    ...
    while (1)
    {
        ...
        {
            // Wait for an event or for the timer to expire
            PROCESS_WAIT_EVENT();

            // If the event timer expired
            if (ev == PROCESS_EVENT_TIMER && data == &freq_timer)
            {
                // Toggle the state of the red LED
                leds_toggle(LEDS_RED);

                // Reset the event timer for the next 1 second
